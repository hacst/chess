...
std::array<uint64_t, 781> polyglot_zobrist_constants = {
...
struct BookEntry {
    ...
    struct Move {
        uint16_t to_file : 3;
        uint16_t to_row : 3;
        uint16_t from_file : 3;
        uint16_t from_row : 3;
        uint16_t promotion_piece : 3;
    } move;
...
template <typename THASH, uint64_t SEED>
IncrementalZobristHasher<THASH, SEED>::IncrementalZobristHasher() {

}

template <typename THASH, uint64_t SEED>
IncrementalZobristHasher<THASH, SEED>::Constants::Constants() {
    sideToMode = 5ULL;
}